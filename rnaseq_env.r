...
## SEASTAR to analyze 
## Systematic Evaluation of Alternative transcription STArt site in RNA
#target[, sortedByCoordBam := paste0('rnaseq/', sortedByCoordBam)]
## SEASTAR need bowtie algined bam files
target[, fastq1.zcatfile := file.path(sgrDir, sub(".gz", "", fastq1))]
target[, fastq2.zcatfile := file.path(sgrDir, sub(".gz", "", fastq2))]

target[, zcat.jobname := paste0("zcat.", sampleName, '.', fastq1)]
target[, zcat.cmd1 := bsub.head(zcat.jobname, mem=1, cpu=1, We='6:59', cwd)]
target[, zcat.cmd1:= paste0(zcat.cmd1, ' "/bin/zcat ', fastq1.full, ' > ', fastq1.zcatfile, '"')]
target$zcat.cmd1[1]

target[, zcat.jobname := paste0("zcat.", sampleName, '.', fastq2)]
target[, zcat.cmd2 := bsub.head(zcat.jobname, mem=1, cpu=1, We='6:59', cwd)]
target[, zcat.cmd2 := paste0(zcat.cmd2, ' "/bin/zcat ', fastq2.full, ' > ', fastq2.zcatfile, '"')]
target$zcat.cmd2[1]

exe.jobs(target$zcat.cmd1, logger)
exe.jobs(target$zcat.cmd2, logger)

## convert quality score
target[, fastq1.cqsfile := file.path(sgrDir, sub(".gz", ".cqs", fastq1))]
target[, fastq2.cqsfile := file.path(sgrDir, sub(".gz", ".cqs", fastq2))]
target$fastq1.cqsfile[1]

target[, cqs.jobname := paste0("cqs.", sampleName, '.', fastq1)]
target[, cqs.cmd1 := bsub.head(cqs.jobname, mem=1, cpu=1, We='2:59', cwd, zcat.jobname)]
target[, cqs.cmd1 := paste0(cqs.cmd1, ' "', ConvertQualityScore, ' --input ', fastq1.zcatfile, ' --output ', fastq1.cqsfile, '"')]
target$cqs.cmd1[1]

target[, cqs.jobname := paste0("cqs.", sampleName, '.', fastq1)]
target[, cqs.cmd2 := bsub.head(cqs.jobname, mem=1, cpu=1, We='2:59', cwd, zcat.jobname)]
target[, cqs.cmd2 := paste0(cqs.cmd2, ' "', ConvertQualityScore, ' --input ', fastq2.zcatfile, ' --output ', fastq2.cqsfile, '"')]
target$cqs.cmd2[1]

for(i in 1:nrow(target)){system(target$cqs.cmd1[i])}
for(i in 1:nrow(target)){system(target$cqs.cmd2[i])}
...
target[, read.len := sapply(target$fastq1.full, read.len)]
target$read.len

target[, fastq1.cutadaptfile :=  file.path(sgrDir, sub(".gz", ".cutadapt", fastq1))]
target[, fastq2.cutadaptfile :=  file.path(sgrDir, sub(".gz", ".cutadapt", fastq2))]
target$fastq1.cutadaptfile[1]

target[, cutadapt.jobname := paste0("cutadapt.", sgrTag)]
target[, cutadapt.cmd := bsub.head(cutadapt.jobname, mem=1, cpu=1, We='2:59', cwd, postdone = cqs.jobname)]
target[, cutadapt.cmd := paste(cutadapt.cmd, ' "', PYTHON, CUTADAPT, '-f fastq -a', clipR1, '-A', clipR2, '--overlap 10 --minimum-length', floor(read.len/2))]
target[, cutadapt.cmd := paste(cutadapt.cmd, '--quality-cutoff', bqtrim, '-o', fastq1.cutadaptfile, '--paired-output', fastq2.cutadaptfile, fastq1.cqsfile, fastq2.cqsfile, ' "')]
target$cutadapt.cmd[1]
...
target[, 
target$R1
target[, patients := sub("_T.*", "", bases)]
target[, samples := sub(".*_(T.*)", "\\1", bases)]
target[, .(patients, samples, sortedByCoordBam, )]
seastar = dcast(target[, .(patients, samples, sortedByCoordBam)], patients ~ samples, value.var = 'sortedByCoordBam')
seastar = seastar[!is.na(T2), ]
seastar[, bases := paste0(patients, '_seastar')]
seastar[, out := paste0('rnaseq/seastar/', patients)]
seastar[, {system(paste0('mkdir -p ', out))}, by=1:nrow(seastar)]
seastar[, seastar.jobname := paste0(patients, '.seastar')]
seastar[, seastar.cmd := bsub.head(seastar.jobname, mem=15, cpu=9, We='20')]
seastar[, seastar.cmd := paste0(seastar.cmd, ' \"bash ~/program/SEASTAR/SEASTAR.sh ')]
seastar[, seastar.cmd := paste0(seastar.cmd, '-A ', T1, ' ')]
seastar[, seastar.cmd := paste0(seastar.cmd, '-B ', T2, ' ')]
seastar[, seastar.cmd := paste0(seastar.cmd, '-o ', out, ' -g ', cfg$genomeGRCh38GTF, ' -i ', cfg$genomeGRCh38chromsize, ' -s ', cfg$genomeGRCh38Fasta, ' -c 0.1 -p 9 \"') ]
seastar$seastar.cmd[1]

exe.jobs(seastar$seastar.cmd, logger)
...
for(gs.name in unique(fl.dt$geneset)){
	...
	gg = ggplot(tmp, aes(x=reorder(name, NES), y=NES, group = name)) + geom_jitter(width=0.2, height=.05) +
