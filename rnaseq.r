...
## group
{
	system('find . -name "*.fastq.gz" > fastq.list')
	fread('fastq.list', header=F) -> target
	...
	target[, fastq1.file := R1]
	target[, fastq2.file := R2]
	target


	fread(target.file) -> target

	target[, rsem.jobname := paste0(base, ".rsem")]
	target[, rsem.cmd := bsub.head(rsem.jobname, mem=55, cpu = 9)]
	target[, rsem.cmd := paste0(rsem.cmd,  " \"", RSEM, "/rsem-calculate-expression  --paired-end -p 8 --star --star-path ", STAR_DIR,  " --gzipped-read-file --append-names --estimate-rspd --output-genome-bam ")]
	target[, rsem.cmd := paste0(rsem.cmd, fastq1.file, " ", fastq2.file, " ",  RSEM_REF, " ", input, "/", base, " \"")]
	target[, rsem.cmd]
...
## import reads 
{
	target[, countfile := paste0(cfg$initDir, '/', base, '.genes.results')]
	file.exists(target$countfile)

	##
	target[, countfile := paste0(input, '/', base, '.genes.results')]
	file.exists(target$countfile)
...
## gsea
{
	...
	fl.dt[, name := NAME]
	fl.d[, name := sub("HALLMARK_", "", NAME)]
	fl.dt[, name := gsub("_", " ", name)]
	fl.dt[, cellshrna := paste0(cell, ', shRNA ', shrna)]
	fl.dt

	for(gs.name in unique(fl.dt$geneset)){
		for(icell in unique(fl.dt$cell)){
			tmp = fl.dt[grep(gs.name, geneset),][grep(icell, cell), ][qval < 0.001 & abs(NES) > 2,]
			if(nrow(tmp) > 30){
				tmp1 = tmp[ES > 0, ][1:15,]
				tmp2 = tmp[ES < 0, ][1:15,]
				tmp = rbind(tmp1, tmp2)
			...
			ww = max(nchar(tmp$name))*.1 + 5; ww
			fname = paste0('r_fang/res/gsea_', gs.name$cell, '_', gs.name$gene, '_', gs.name$geneset, '.pdf')
			ggsave(gg, file=fname, width=ww, height=8)
		}
	}
	sync('res')
	sync('r_fang/res')
...
## raw counts
{
	...
	nm.file = paste0(cfg$geoDir, '/normalized_counts.tsv')
	write.table(tmp, file=nm.file, sep="\t", quote=F)
	res.raw.cc = counts(dds, normalized=T);
	rw.file = paste0(cfg$geoDir, '/raw_counts.tsv')
	write.table(tmp, file=rw.file, sep="\t", quote=F)
	system(paste0('md5sum ', nm.file, ' > ', cfg$geoDir, '/md5.txt'))
	system(paste0('md5sum ', rw.file, ' >> ', cfg$geoDir, '/md5.txt'))

	scan('fastq.list', character()) -> tmp
	cmd = paste0('md5sum ', tmp, ' >> ', cfg$geoDir, '/md5.txt')
	exe.jobs(cmd, logger)
}
...
## export counts table
## geo submission need this
{
	write.table(res.cc, file='res/counts_table.tsv', sep="\t", quote=F)
}
...
{
	library(ggfortify)
	gg = autoplot(prcomp(t(mtx)), data = mmu.uro.targets, colour = 'group', scale=0)
	ggsave(gg, file=paste0('res/gg_pca.pdf'), width=4, height=3)
	sync('res')
...
## ssGSEA 
{
	...
	gsva.score  = gsva(res.cc, geneset, method="ssgsea")
	gsva.score
