...
## combine with tcga blca ## squamous cell and basal cells
...
w = 8; h = 8 
pdf('exonseq/maftools/oncoplot_t1.pdf', width=w, height=h)
oncoplot(maf = scc.maf.t1.o, genes=mut.2017.sel, color = color_vector, showTumorSampleBarcodes=F, top=20)
dev.off()
pdf('exonseq/maftools/oncoplot_t2.pdf', width=w, height=h)
...
## combine with tcga blca 
## squamous cell and basal cells
tcga.blca.clin <- GDCquery_clinic("TCGA-BLCA", type = "clinical", save.csv = TRUE)
colnames(tcga.blca.clin)
tcga.blca.clin$bcr_patient_barcode

load('../bcg/blcaExp.rda')
tcga.blca.htseq.raw.mtx = data; rm(data)
rsem.files = paste0('rnaseq/', gsub("-", "_", incl.id), '/',  gsub("-", "_", incl.id), '.genes.results')
file.exists(rsem.files)
rsem

rsem = tximport(rsem.files, type = "rsem")
rsem.counts = rsem$counts
colnames(rsem.counts) = incl.id
rsem.counts[1:10,1:5]
dim(rsem.counts)
mode(rsem.counts) <- "integer"
symbol.ensembl = data.frame(row.names=row.names(rsem.counts), stringsAsFactors = F)
symbol.ensembl$symbol = sub(".*_", "", row.names(rsem.counts))
symbol.ensembl$ensembl= sub("_.*", "", row.names(rsem.counts))
row.names(symbol.ensembl) = symbol.ensembl$ensembl
head(symbol.ensembl)

tcga.blca.htseq.raw.mtx[1:10,1:10]
dim(tcga.blca.htseq.raw.mtx)
row.names(rsem.counts) = sub("_.*", "", row.names(rsem.counts))
ov = intersect(row.names(rsem.counts), row.names(tcga.blca.htseq.raw.mtx))
length(ov)

cmb = cbind(tcga.blca.htseq.raw.mtx[ov, ], rsem.counts[ov, ])
cmb[1:10,1:10]

design = data.frame(row.names       = colnames(cmb),
		    libType         = rep("PE", ncol(cmb)));
design$condition = c(rep('tcga', ncol(tcga.blca.htseq.raw.mtx)), rep('scc', ncol(rsem.counts)))
design

ddsmat = DESeqDataSetFromMatrix(countData = cmb,
				colData = design,
				design = ~ condition)
...
cmb.cc = counts(dds, normalized = T)

substr(colnames(cmb.cc)[1:433], 1, 12) -> bcr
bcr = c(bcr, incl.id)
anno.col = data.frame(row.names = colnames(cmb.cc), stringsAsFactors = F)
anno.col$class = 'Normal'
anno.col$class[bcr %in% incl.id] = 'T1/T2'
anno.col$class[bcr %in% tcga.blca.clin$bcr_patient_barcode] = 'urothelia'
anno.col$class[bcr %in% tcga_basal_id] = 'basal'
anno.col$class[bcr %in% tcga_sc_id] = 'squamous'
anno.col$class

## pca
tmp = apply(cmb.cc, 1, sd)
tmp = cmb.cc[order(tmp, decreasing = T), ]
tmp = tmp[1:1000,]
tmp = prcomp(t(tmp))
tmp = tmp$x
tmp = as.data.frame(tmp)
tmp$class = anno.col$class

pdf("rnaseq/res/pca_tcga_blca_T1T2.pdf", width=8, height=6)
ggplot(tmp, aes(PC1, PC2, color = class)) + geom_point() + theme(legend.position="right") 
dev.off()
sync('rnaseq/res')

mtx = cmb.cc[base47.ensembl,]
mtx = log2(mtx+1)
row.names(mtx) = symbol.ensembl[row.names(mtx), 'symbol']
mtx
anno.row = data.frame(row.names = row.names(mtx), stringsAsFactors = F)
anno.row$geneClass = 'basal'
anno.row$geneClass[row.names(anno.row) %in% base47.luminal] = 'luminal' 
anno.row

cn = colnames(mtx)
tmp = t(apply(mtx, 1, scale))
colnames(tmp) = cn
tmp[tmp > 2] = 2
tmp[tmp < -2] = -2

pdf("rnaseq/res/heatmap_base47_tcga_blca_T1T2_v2.pdf", width=60, height=8)
hp = pheatmap(tmp, color=greenred(length(breaks)+1), show_colnames = T,
	      scale='none', show_rownames = T, breaks = breaks, 
	      clustering_method = 'complete', 
	      cluster_rows = TRUE, cluster_cols = TRUE, fontsize_row = 6,
	      annotation_col =anno.col, annotation_row = anno.row)
dev.off()
sync('rnaseq/res')


mtx = cmb.cc[mcconkey.ensembl,]
mtx = log2(mtx+1)
row.names(mtx) = symbol.ensembl[row.names(mtx), 'symbol']
mtx
...
anno.row = data.frame(row.names = row.names(mtx), stringsAsFactors = F)
anno.row$geneClass = 'basal'
anno.row$geneClass[row.names(anno.row) %in% mcconkey.luminal] = 'luminal' 
anno.row
...
pdf("rnaseq/res/heatmap_mcconkey_tcga_blca_T1T2_v2.pdf", width=60, height=7)
hp = pheatmap(tmp, color=greenred(length(breaks)+1), show_colnames = T,
	      scale='none', show_rownames = T, breaks = breaks, 
	      clustering_method = 'complete', 
	      cluster_rows = TRUE, cluster_cols = TRUE, fontsize_row = 6,
...
## gene list of cell paper
## http://www.cell.com/cell/fulltext/S0092-8674(17)31056-5
## source('~/program/fun/geneSampleList.r')
mtx = cmb.cc[tcga.blca.marker$ensembl,]
mtx = log2(mtx+1)
row.names(mtx) = symbol.ensembl[row.names(mtx), 'symbol']
mtx

anno.row = data.frame(row.names = tcga.blca.marker$symbol, stringsAsFactors = F)
anno.row$geneClass = tcga.blca.marker$Class
anno.row
...
pdf("rnaseq/res/heatmap_tcga_blca_T1T2_v2.pdf", width=60, height=7)
hp = pheatmap(tmp, color=greenred(length(breaks)+1), show_colnames = T,
#pdf("rnaseq/res/heatmap_tcga_blca_T1T2.pdf", width=10, height=7)
#hp = pheatmap(tmp, color=greenred(length(breaks)+1), show_colnames = F,
	      scale='none', show_rownames = T, breaks = breaks, 
	      clustering_method = 'complete', 
	      cluster_rows = TRUE, cluster_cols = TRUE, fontsize_row = 6,
	      annotation_col =anno.col, annotation_row = anno.row)
